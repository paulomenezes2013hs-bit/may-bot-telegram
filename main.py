...
DONO_USERNAME = "catalogoseriesfilmes" # Mude para seu user sem o @
...
async def enviar_pix(chat_id):
    ...
    texto = (
        "✅ **Que bom que você já escolheu sua Série/Anime, manda no privado o nome!**\n\n"
        "💰 **Pix:** `81991172344` (Toque para copiar)\n"
        "**Valor:** `R$: 6,00` / **Promoção 3 Séries/animes** `R$: 15,00`\n\n"
...
def webhook():
    ...
    update = Update.de_json(dados, bot)
    
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    
    try:
        membro = update.chat_member or update.my_chat_member
        
        # 1. Boas-vindas para novos membros
        if membro and membro.new_chat_member.status == "member":
            chat_id = membro.chat.id
            nome = membro.new_chat_member.user.first_name
            loop.run_until_complete(enviar_boas_vindas(chat_id, nome))
            
        # 2. Resposta com o novo texto do Pix
